// ============================================================================
//  DIY Reflow Oven Controller — Compact Flat Bench Enclosure (v9.10, X1C Fit)
//  Parametric OpenSCAD model for Bambu X1C (PETG minimum, ASA preferred)
//
//  v9.10 CHANGE: Fixed window shift to 1mm instead of 3mm. Real offset is 1mm toward ESP32 end.
//                Fixed 2-manifold warnings by adding microscopic overlaps to all bosses.
//
...
//  relative to the hole pattern (5mm clipped there, 0mm at USB end).
//  Symmetric coverage is impossible (needs 70mm > 69mm glass).
//  BOARD NOW INSTALLS IN ONE ORIENTATION: ESP32/flex end toward the front (y=0) wall.
//
...
//  MEASURED 64mm viewable region (3mm ESP32 end / 1mm USB end).
//  66mm symmetric is the minimal window covering the viewable area in either board rotation.
//
//  v9.6 CHANGE: 3Dman IEC cutout right-sized to the MEASURED insert body
//  (47 x 27.5 x 21.5mm): cutout 52x38 -> 48x28.5.
//  The old oversize left open gaps past the tapered flange edge.
//  Added iec_trim_plate retrofit part for shells already printed with the old cutout.
//
...
//  4mm from edges of the 86x50 board).
//  Added cyd_test_coupon part.
//
...
//
//  NOTE: This is enclosure geometry only, not wiring approval.
//  Verify current ratings, fuse value, wire gauge, strain relief, grounding, creepage/clearance,
...
/* [Render selector] */
part = "assembly"; // ["bottom","lid","bezel","assembly","all_flat","iec_test_coupon","kp200_test_coupon","cyd_test_coupon"]

/* [Box outer dimensions]
   220 x 155 is the compact X1C-friendly footprint.
   It leaves room on a 256 x 256mm Bambu bed for skirt/brim and avoids pushing the printable edge.
...
/* [AC side: 3Dman fused IEC C14 inlet + rocker/fuse module]
   ...
   This is the device input socket, not the smart shutoff.
   Dimensions are based on common 3Dman/Amazon-style listings:
   approx face/body 58 x 48 x 34mm.
   Print the coupon and measure your part. */
...
inlet_cutout_w = 48;        // v9.6: was 52. MEASURED body is 47mm long; 0.5mm/side clearance.
                            // Old oversize left gaps past the tapered flange
inlet_cutout_h = 28.5;      // v9.6: was 38. MEASURED body is 27.5mm tall; 0.5mm/side.
                            // Old 38 left ~5mm exposed gaps above/below body
inlet_center_x = 118;
inlet_center_z = 31;
...
/* [AC side: KP200 smart outlet / smart shutoff]
   Top-lid mount.
   The KP200 face/yoke footprint is treated as 85 x 129mm.
...
kp_window_y = 73;           // v9.2: was 70;
                            // widened toward 74mm hard ceiling (boss gap) for KP200 body insertion           // shorter than screw spacing so screw bosses land on solid lid
kp_lid_boss_h = 7;
kp_lid_boss_dia = 9;
kp_insert_dia = 4.5;
kp_insert_depth = 5;
...
/* [DC side: CYD top-lid mount]
   ...
   This saves about 25-30mm of total enclosure width.
   Set cyd_rotate_90_for_compact = false only if you want the earlier landscape
   physical mounting orientation;
   you will likely need to widen box_w. */
cyd_rotate_90_for_compact = true;
cyd_raw_pcb_w = 90;
cyd_raw_pcb_h = 60;
cyd_raw_hole_w = 77.5;      // v9.5: was 82 (assumed). MEASURED center-to-center on the physical board: 77.5mm.
                            // (DIYmall drawing implies 78; measured board wins — clone variants differ)
cyd_raw_hole_h = 42;        // v9.5: was 44 (assumed).
                            // MEASURED center-to-center on the physical board: 42mm
cyd_raw_display_w = 65;     // v9.8: was 66 (v9.7), 60 before.
                            // MEASURED: viewable region is 64mm and sits OFFSET
                            ...
                            // 65mm + 3mm shift covers it with 0.5mm margin per end.
                            // A symmetric window can no
                            // longer work: full coverage would need 70mm > the 69mm glass.

cyd_window_shift = 1;       // v9.10: CHANGED from 3 to 1 based on actual 8mm vs 6mm measurements.
...
cyd_raw_display_h = 48;    // v9.4: was 46 (= glass width);
                           // +1mm/side because measured glass sits 0.5-1mm off-center between mounting holes.
                           // Symmetric widening is rotation-agnostic (hole pattern lets the board install either way).
...
cyd_lid_boss_dia = 8.5;
// v9.2: was 7; 1.4mm insert wall too thin, now 2.15mm
cyd_insert_dia = 4.2;
cyd_insert_depth = 4;
cyd_screw_clear_dia = 3.2;

/* [DC side: USB-C access slot]
   Side-wall slot aligned with the CYD area.
   Exact position depends on your
...
echo("=== Compact flat bench enclosure v9.10 / X1C fit / KP200 + 3Dman IEC ===");
    ...
    difference() {
        union() {
            ...
            for (p = boss_positions)
                translate([p[0], p[1], floor_t - eps]) lid_boss(inner_h - 0.2 + eps);

            // MAX6675 standoff bosses on DC floor
            for (dx = [-max_hole_dx / 2, max_hole_dx / 2])
                for (dy = [-max_hole_dy / 2, max_hole_dy / 2])
                    translate([max_pos_x + dx, max_pos_y + dy, floor_t - eps]) max_boss(5 + eps);
        }
    ...
    difference() {
        union() {
            ...
            for (p = cyd_hole_positions)
                translate([p[0], p[1], lid_t - eps]) cyd_lid_boss(cyd_lid_standoff_h + eps);

            // KP200 yoke screw bosses on inside of lid
            for (p = kp_screw_positions)
                translate([p[0], p[1], lid_t - eps]) kp_lid_boss(kp_lid_boss_h + eps);
        }
...
module iec_trim_plate() {
    ...
    difference() {
        ...
        translate([cx - inlet_cutout_w / 2, cy - inlet_cutout_h / 2, -eps])
            cube([inlet_cutout_w, inlet_cutout_h, 1.5 + 2 * eps]);
        for (dy = [-inlet_screw_spacing / 2, inlet_screw_spacing / 2])
            translate([cx, cy + dy, -eps])
                cylinder(d = inlet_screw_dia, h = 1.5 + 2 * eps);
    }
}
    ...
    difference() {
        union() {
            ...
            for (dy = [-kp_screw_spacing / 2, kp_screw_spacing / 2])
                translate([cx, cy + dy, coupon_t - eps]) kp_lid_boss(kp_lid_boss_h + eps);
        }
        translate([cx - kp_window_x / 2, cy - kp_window_y / 2, -eps])
...
module cyd_test_coupon() {
    cw = cyd_display_w + 2 * (cyd_lid_boss_dia + 4);
    ch = cyd_hole_h + cyd_lid_boss_dia + 10;
    cx = cw / 2; cy = ch / 2;
...
module lid_boss(h_val) {
    difference() {
        cylinder(d = boss_outer_dia, h = h_val);
        translate([0, 0, h_val - boss_insert_depth + eps])
            cylinder(d = boss_insert_dia, h = boss_insert_depth + eps);
    }
}

module cyd_lid_boss(h) {
    difference() {
...
module component_previews() {
    if (show_components) {
        color("DimGray", component_alpha)
            translate([ssr_center_x - heatsink_w / 2, ssr_center_y - heatsink_d / 2, floor_t])
                cube([heatsink_w, heatsink_d, heatsink_h]);
        color("Purple", component_alpha)
            translate([kp_center_x - kp_wid_x / 2, kp_center_y - kp_len_y / 2, box_h + 0.2])
                cube([kp_wid_x, kp_len_y, 1.5]);
        color("MediumPurple", component_alpha)
            translate([kp_center_x - kp_wid_x / 2, kp_center_y - kp_len_y / 2, box_h - lid_t - kp_depth_z])
                cube([kp_wid_x, kp_len_y, kp_depth_z]);
        color("ForestGreen", component_alpha)
            translate([cyd_center_x - cyd_pcb_w / 2, cyd_center_y - cyd_pcb_h / 2, box_h - lid_t - cyd_lid_standoff_h - 1.6])
                cube([cyd_pcb_w, cyd_pcb_h, 1.6]);
        color("DarkGreen", component_alpha)
            translate([max_pos_x - max_board_w / 2, max_pos_y - max_board_h / 2, floor_t + 5])
                cube([max_board_w, max_board_h, 1.6]);
        color("Crimson", component_alpha)
            translate([inlet_center_x - inlet_face_w / 2, -1.5, inlet_center_z - inlet_face_h / 2])
                cube([inlet_face_w, 1.5, inlet_face_h]);
        color("DarkRed", component_alpha)
            translate([inlet_center_x - inlet_cutout_w / 2, wall_t, inlet_center_z - inlet_cutout_h / 2])
                cube([inlet_cutout_w, inlet_body_depth, inlet_cutout_h]);
        color("OrangeRed", component_alpha)
            translate([inlet_center_x - inlet_cutout_w / 2, wall_t + inlet_body_depth, inlet_center_z - inlet_cutout_h / 2])
